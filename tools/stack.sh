#!/usr/bin/env bash
#
# tools/stack.sh — EIN wiederholbarer Aufbau des kompletten Monorepos für die Kundenpräsentation.
# ====================================================================================================
...
#
# AUFRUF
#   bash tools/stack.sh                  # voller Lauf (reset → ... → summary)
#   bash tools/stack.sh --liste          # alle Schritte auflisten
#   bash tools/stack.sh --from test-java # ab Schritt 'test-java' (Stack bleibt wie er ist)
#   bash tools/stack.sh --only seed      # nur den Seed-Schritt
#   bash tools/stack.sh --keine-reset    # Volumes NICHT wipen (schnelles Re-Seed/-Test)
#
...
warn()  { echo "${gelb}⚠${aus} $*"; }
err()   { echo "${rot}✗ $*${aus}" >&2; }
fail()  { err "$*"; echo "${rot}${fett}Abbruch.${aus} Stack bleibt oben — fortsetzen mit: bash tools/stack.sh --from <schritt>" >&2; exit 1; }
...
schritt_preflight() {
  ...
  command -v py >/dev/null 2>&1 && ok "py (Python-Launcher)" || warn "py fehlt — docs/prozessdoku/build.sh nutzt py -3.13"
  # OpenOLAT-Build-Artefakte (gitignored) — ohne sie kann das openolat-Image nicht bauen.
  if ls services/openolat/openolat_*.war >/dev/null 2>&1 && [ -f services/openolat/postgresql.jar ] && [ -f services/openolat/dart-sass.tar.gz ]; then
    ok "OpenOLAT-Artefakte vorhanden"
  else
    fail "OpenOLAT-Artefakte fehlen → erst: bash services/openolat/fetch-artifacts.sh"
  fi
...
schritt_seed() {
  ...
  ( cd services/vendure && node scripts/seed-demo-orders.mjs ) || fail "seed-demo-orders.mjs fehlgeschlagen"
  ok "Demo-Bestellungen geseedet"

  echo "── Portal-Rechnungen (Firmen-/Privat-Beleg + ZUGFeRD) ──"
  bash services/integration/smoke-portal-rechnungen.sh || fail "smoke-portal-rechnungen.sh fehlgeschlagen"
  ...
  if [ -d "$scorm" ] && [ -n "$(ls -A "$scorm" 2>/dev/null)" ]; then
    bash services/openolat/lms-import-seed.sh || warn "lms-import-seed.sh fehlgeschlagen — Kurse evtl. nicht importiert"
    ok "LMS-Kurse importiert"
  fi

  echo "── Controlling-Warehouse (dlt: vendure → controlling) ──"
  ( cd data/dlt && .venv/Scripts/python vendure_to_warehouse.py ) || fail "dlt-Load fehlgeschlagen"
...
schritt_mandanten_seed() {
  ...
  # ebz-kunde-demo) liegen als Import-JSON in services/vendure/keycloak/realms/ und sind beim frischen Boot da.
  ...
  bash infra/seeds/mandanten-seed/seed-mandanten.sh || fail "Mandanten-Seed fehlgeschlagen"
...
schritt_lms_share() {
  ...
  bash services/openolat/lms-share-seed.sh || fail "Content-share-once-Seed fehlgeschlagen"
...
schritt_lms_nachweis() {
  ...
  bash services/openolat/lms-nachweis-seed.sh || fail "Nachweis-Seam-Seed fehlgeschlagen"
...
schritt_lms_zertifikat() {
  ...
  bash services/openolat/lms-zertifikat-seed.sh || fail "Auto-Zertifikat-Seed fehlgeschlagen"
...
schritt_test_java() {
  ...
  "$MVN" -q -f services/integration/pom.xml test || fail "Java-Integrationstests rot"
  ok "Java-Integrationstests grün"
}

schritt_test_spa() {
  phase "TEST — SPAs (vitest + typecheck)"
  for p in apps/mdm apps/portal; do
    echo "── $p ──"
  ...
  ( cd apps/storefront && pnpm install --frozen-lockfile >/dev/null 2>&1 || pnpm install >/dev/null 2>&1; \
...
schritt_storybook() {
  ...
  for sb in apps/design-system/crm-kernmaske apps/design-system/kunden-kernmaske; do
    echo "── $sb ──"
...
schritt_test_vendure() {
  ...
  for s in smoke-shop smoke-subscriptions smoke-rechnungslauf smoke-sso smoke-checkout-f1; do
    echo "── $s ──"
    ( cd services/vendure && node "scripts/$s.mjs" ) || fail "services/vendure/$s rot"
    ok "$s grün"
  done
}
...
schritt_test_e2e() {
  ...
  ( cd tests/e2e && pnpm install --frozen-lockfile >/dev/null 2>&1 || pnpm install >/dev/null 2>&1; \
...
schritt_bpmn() {
  phase "BPMN — Living Documentation (OTel-Spans → PM4py → docs/bpmn/)"
  bash docs/prozessdoku/build.sh || fail "BPMN-Generierung fehlgeschlagen"
  ok "BPMN aktualisiert (docs/bpmn/)"
}
...
schritt_lightdash() {
  ...
  ( cd data/dbt && LIGHTDASH_URL="$LD" .venv/Scripts/python ../lightdash/bootstrap.py ) \
...
${fett}Zugänge / Demo-URLs:${aus}
  ...
  Adminer (DBs) .............. http://localhost:8082   (Server postgres · services/vendure/controlling/lightdash)
  Mailpit (Mails) ............ http://localhost:8025
  Jaeger (Tracing) ........... http://localhost:16686
  Phoenix (LLM-Tracing) ...... http://localhost:6006

${fett}Telemetrie/BPMN:${aus} docs/bpmn/ · Live-Spans in Jaeger (Service ebz-integration)
${fett}Daten bleiben stehen.${aus} Erneuter sauberer Aufbau: bash tools/stack.sh
EOF
}
