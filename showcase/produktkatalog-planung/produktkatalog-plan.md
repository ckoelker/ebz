...
**P7c E-Learning-Kopplung — BLOCKIERT auf Kunden-SSO:** Einschreibungs-Outbox existiert komplett (`KurseinschreibungService.ausBestellung(vendureOrderId, keycloakSub, email, anzeigeName, vendureProductIds)` + `POST /lms/einschreibungen/bestellung`, L0–L3). Aber `ausBestellung` verlangt **keycloakSub**; die neue Storefront ist Gast-only (Kunden-SSO in P2–P6 zurückgestellt) → kein Lerner-Sub. Trigger erst sinnvoll, wenn (a) Kunden-SSO in der Storefront ergänzt ODER (b) email-basierte provisorische OpenOLAT-Provisionierung im LMS-Service nachgezogen wird. Nutzer-Entscheidung ausstehend.
