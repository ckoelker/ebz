...
**P7b CRM→Vendure-Personen-Sync ✅ GEBAUT+verifiziert** (Commit d3ba7ae): `POST /shop/crm-personen-sync` (Rolle katalog-pflege) projiziert aktive EBZ-`Mitarbeiter` (Party-Kern bleibt SoR) idempotent als Vendure-`Ansprechpartner` (crmPersonId=`mitarbeiter-<id>`) via bestehende `upsertAnsprechpartner`-Mutation. Test 2/2. (Dozenten bleiben Seed; aktuell 0 aktive Mitarbeiter im Datenbestand → gesendet=0, Mechanismus verifiziert.)
**Kunden-SSO ✅ GEBAUT+verifiziert** (Commit 58f357e): Keycloak-Login (Realm ebz-customers) in der Storefront — Token im Browser geholt (Issuer localhost:8088), serverseitig an Vendure-Shop `authenticate` gereicht → Customer an Session gebunden; `useAuth` + Header-Login + Kasse-Prefill. `shopGql` In-Request-Token-Kontinuität gefixt. (Showcase: ROPC; Produktiv = Code-Flow.)
**P7c E-Learning-Trigger ✅ GEBAUT+verifiziert (Mechanismus)** (Branch feature/produktkatalog-p1): nach bezahltem Checkout ruft die Storefront (nur eingeloggte Kunden, kc_sub-Cookie) `POST /lms/einschreibungen/bestellung` der Integration — Auth via Keycloak-**Service-Account** `storefront-service` (client_credentials, Issuer keycloak.localhost:8080 = von der Integration erwartet), best-effort. Live verifiziert: eingeloggter WBT-Checkout → Integration-Log „Order …: 0 WBT-Einschreibung(en) aus 1 Position(en)". **Restliche Daten-Lücke für „1 statt 0":** Katalog-Seed-WBT-Produkt ≠ LMS-projiziertes WbtKurs-Produkt (LMS legt eigenes Produkt via `POST /lms/kurse/{id}/veroeffentlichen` an). Voll-Einschreibung erfordert, dass das buchbare E-Learning-Produkt ein LMS-projiziertes ist (Seed-/Architektur-Angleichung) — separater Schritt.
