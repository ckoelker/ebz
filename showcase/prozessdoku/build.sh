...
#   E2E-Tests → OTel-Span-Log → PM4py (BPMN) → bpmn-auto-layout → showcase/docs/bpmn/
#
# Aufruf:
...
mvn -q -f ../integration/pom.xml test -Dtest='*E2ETest'
...
echo "==> 3/3 layout.mjs (bpmn-auto-layout → ../docs/bpmn/)"
[ -d node_modules ] || pnpm install
node layout.mjs

if [ "${1:-}" = "--check" ]; then
  echo "==> CI-Verify: git diff der generierten BPMN"
  git diff --exit-code -- ../docs/bpmn/
fi
echo "OK: Prozessdoku aktualisiert (showcase/docs/bpmn/)."
