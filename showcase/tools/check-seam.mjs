...
//
// Lauf: `node showcase/tools/check-seam.mjs` (exit 1 bei Verstoß). Auch als Schritt `naht` in showcase-aufbau.sh.
import { readdir, readFile } from 'node:fs/promises';
import { fileURLToPath } from 'node:url';
import path from 'node:path';

const SHOWCASE = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
...
for (const rule of RULES) {
  ...
  for (const root of rule.roots) {
    for await (const file of walk(path.join(SHOWCASE, root))) {
      const lines = (await readFile(file, 'utf8')).split(/\r?\n/);
      lines.forEach((line, i) => {
        for (const { spec, re } of tests) {
          if (re.test(line)) {
            violations.push({ rule, spec, file: path.relative(SHOWCASE, file), line: i + 1, text: line.trim() });
          }
        }
      });
    }
