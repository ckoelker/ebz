...
import { MITARBEITER, GRUPPEN } from '../../mock/data'
...
<template>
  <div>
    <UButton color="primary" variant="soft" icon="i-lucide-calendar-clock" @click="open = true">Wiedervorlage</UButton>

    <UModal v-model:open="open" title="Wiedervorlage anlegen" :ui="{ content: 'max-w-xl' }">
      <template #body>
        <div class="space-y-4">
      ...
      <template #footer>
        <div class="flex justify-end w-full gap-2">
          ...
          <UButton color="primary" icon="i-lucide-check" :disabled="!form.betreff" @click="emit('save', form); open = false">Anlegen</UButton>
        </div>
      </template>
    </UModal>
  </div>
