...
import { PERSONEN, personName, initialen, mitgliedschaftenVonPerson, orgById } from '../../mock/data'

// CTI-Demo (anbieter-neutral, später WebSocket): eingehender Anruf unten rechts.
// Bekannte Nummer → Treffer mit Direktsprung; unbekannt → Suche/Neuanlage anbieten.
const props = defineProps<{ nummerE164: string }>()
const emit = defineEmits<{ (e: 'oeffnen', id: string): void; (e: 'neu'): void; (e: 'schliessen'): void }>()

const person = computed(() => PERSONEN.find(p => p.kontaktpunkte.some(k => k.nummerE164 === props.nummerE164)))
const firma = computed(() => {
  if (!person.value) return null
  const haupt = mitgliedschaftenVonPerson(person.value.id).find(m => m.hauptzugehoerigkeit)
  return haupt ? orgById(haupt.orgId) : null
})
</script>
    ...
    <div class="p-4 space-y-3">
      <template v-if="person">
        <div class="flex items-center gap-3">
          <UAvatar :text="initialen(personName(person))" size="md" />
          <div>
